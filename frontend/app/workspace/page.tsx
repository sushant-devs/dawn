...
export default function WorkspacePage() {
  ...
  const renderDonutLabel = ({
    ...
    const endX = cx + (outerRadius + 16) * Math.cos(-midAngle * RADIAN);
    const endY = cy + (outerRadius + 16) * Math.sin(-midAngle * RADIAN);
    const isRightSide = endX >= cx;
    const textX = endX + (isRightSide ? 5 : -5);
    const textAnchor: 'start' | 'end' = isRightSide ? 'start' : 'end';

    return (
      <g>
        <line x1={startX} y1={startY} x2={endX} y2={endY} stroke={fill} strokeWidth={1} />
        <text
          x={textX}
          y={endY}
          fill={fill}
  ...
  const renderDonutTooltip = ({
    ...
    return (
      <div className="rounded-[10px] border border-slate-200 bg-white px-3 py-2 text-xs shadow-[0_10px_24px_rgba(15,23,42,0.12)]">
        <p className="font-semibold text-slate-700">{segment?.name ?? 'Segment'}</p>
        <p className="mt-0.5 font-medium text-slate-600">{segment?.value ?? 0}%</p>
      </div>
    );
  };
  ...
  return (
    <main className="h-screen overflow-hidden bg-gradient-to-br from-slate-50 via-white to-blue-50/30 relative">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        ...
        <div className="absolute top-1/2 left-1/2 w-72 h-72 bg-gradient-to-r from-dawn-teal/10 to-[#B47CFF]/10 rounded-full blur-3xl animate-pulse delay-500"></div>
      </div>
          ...
          <div className="mx-auto max-w-7xl space-y-6">
            ...
            <div className="relative">
              ...
              <div className="text-center my-6">
                ...
                <h1 className="font-display text-3xl md:text-4xl bg-gradient-to-r from-dawn-navy via-dawn-teal to-[#B47CFF] bg-clip-text text-transparent mb-3 font-bold leading-tight">
            ...
            {/* Workspace Section */}
            <section className="relative rounded-3xl border border-slate-200/50 bg-white/80 p-4 shadow-md shadow-slate-900/5 backdrop-blur-xl">
              <div className="mb-4">
                <div className="mb-1 flex items-center justify-between gap-3">
                  <div className="flex items-center gap-3">
                    <h2 className="text-xl font-bold text-dawn-navy">Your Workspaces</h2>
                    <div className="inline-flex items-center gap-1.5 rounded-xl border border-emerald-200/50 bg-gradient-to-r from-emerald-50 to-green-50 px-3 py-1 text-xs font-semibold text-emerald-700">
                      <FolderOpen size={12} />
                  ...
                  <button
                    ...
                    className="inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-dawn-teal to-[#B47CFF] px-4 py-2 text-sm font-semibold text-white shadow-md shadow-dawn-teal/30 transition-all duration-200 hover:shadow-lg hover:shadow-dawn-teal/40 hover:-translate-y-0.5 cursor-pointer"
                  >
                    <Plus size={16} />
                    Create Workspace
                  </button>
                ...
                {workspaceList.length === 0 ? (
                  <div className="flex min-h-[300px] flex-col items-center justify-center rounded-3xl border-2 border-dashed border-dawn-teal/30 bg-gradient-to-br from-white/80 to-dawn-sky/20 backdrop-blur-xl p-12 text-center">
                    <div className="relative mb-6">
                      ...
                      <div className="relative flex h-16 w-16 items-center justify-center rounded-2xl bg-gradient-to-br from-dawn-teal to-[#B47CFF] text-white shadow-lg">
                        <Activity size={24} />
                      </div>
                    </div>
                  ...
                  <div className="max-h-[400px] overflow-y-auto pr-1 thin-x-scroll">
                    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3">
                      ...
                      <Link
                        ...
                        className="group relative min-h-[190px] overflow-hidden rounded-2xl border border-slate-200/50 bg-white/90 p-5 shadow-lg shadow-slate-900/5 backdrop-blur-xl transition-all duration-300 hover:-translate-y-1 hover:border-dawn-teal/40 hover:shadow-xl hover:shadow-slate-900/15"
                      >
                        {/* Animated background gradient */}
                        <div className="absolute inset-0 bg-gradient-to-br from-dawn-teal/5 via-transparent to-[#B47CFF]/5 opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>

                        <div className="relative">
                          <div className="flex items-start justify-between mb-3">
                            <div className="relative">
                              <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-br from-dawn-teal to-[#B47CFF] text-white shadow-md group-hover:shadow-lg group-hover:scale-105 transition-all duration-300">
                                <FolderOpen size={16} />
                              </div>
