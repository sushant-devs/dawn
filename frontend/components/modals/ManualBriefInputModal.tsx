...
const PROMPT_AUTOFILL: Record<string, Record<string, string>> = {
  multi_deliverable_campaign_content_brief: {
    CAMPAIGN_NAME: 'Brexiva Global Oncology Engagement Campaign',
    BRAND: 'Brexiva',
    DRUG_NAME: 'Brenova',
    THERAPEUTIC_AREA: 'Oncology',
    MARKETS: 'Global',
    PRIMARY_AUDIENCE: 'Oncologists and Breast Cancer Specialists',
    SECONDARY_AUDIENCE:
      'Oncology nurses, oncology pharmacists, medical affairs teams, and patient advocacy stakeholders',
    CAMPAIGN_OBJECTIVE:
      'Build a global specialist-facing campaign that supports evidence-led education, disease-state awareness, safety understanding, and patient-centered communication for HR+/HER2− metastatic breast cancer.',
    KEY_MESSAGE_THEME:
      'Advancing evidence-informed and patient-centered conversations in HR+/HER2− metastatic breast cancer.',
    DELIVERABLES: 'Email, Digital Detail Aid, Congress Poster, Patient Leaflet',
    CHANNELS:
      'HCP email, digital detailing platform, congress booth, congress poster session, patient advocacy support setting, and downloadable PDF resource.',
    MANDATORY_INCLUSIONS:
      'Brand and drug context, indication, global market scope, target audience, clinical evidence framing, safety and monitoring language, fair-balance language, and MLR review readiness.',
    TONE: 'Scientific, balanced, specialist-facing',
  },
};
...
export default function ManualBriefInputModal({ onConfirm, onClose, readOnly = false }: ManualBriefInputModalProps) {
  ...
  const categories = ['All', 'Congress Engagement', 'Integrated Campaign', 'Launch Campaign', 'Omnichannel Engagement', 'Patient Support', 'Product Education'];
  ...
  const selectPrompt = (prompt: MedicalPrompt) => {
    ...
    const presets = PROMPT_AUTOFILL[prompt.id];
    const initialVariables: Record<string, string> = {};
    prompt.variables.forEach(v => {
      // Pre-fill from the per-prompt autofill preset when available, otherwise blank.
      initialVariables[v] = presets?.[v] ?? '';
    });
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-5xl max-h-[92vh] bg-white flex flex-col rounded-2xl border border-[#e2e8f7] shadow-[0_24px_64px_rgba(15,23,42,0.2)] animate-scale-in">
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-5 border-b border-dawn-border bg-white rounded-t-2xl">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="font-serif text-xl text-dawn-navy font-semibold">Manual Brief Input</h2>
              <p className="text-sm text-gray-500 mt-0.5">Add your campaign briefs or use Medical Prompt Library</p>
            </div>
          </div>
                      ...
                      >
                        ...
                        <div className="flex items-center gap-2 flex-wrap">
                          <span className={`text-[10px] px-2 py-0.5 rounded-full ${
                            prompt.category === 'Congress Engagement' ? 'bg-blue-100 text-blue-700' :
                            prompt.category === 'Integrated Campaign' ? 'bg-purple-100 text-purple-700' :
                            prompt.category === 'Launch Campaign' ? 'bg-green-100 text-green-700' :
                            prompt.category === 'Omnichannel Engagement' ? 'bg-cyan-100 text-cyan-700' :
                            prompt.category === 'Patient Support' ? 'bg-amber-100 text-amber-700' :
                            prompt.category === 'Product Education' ? 'bg-red-100 text-red-700' :
