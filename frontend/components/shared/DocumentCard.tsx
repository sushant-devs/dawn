...
const TYPE_CONFIG: Record<DocumentCardType['type'], { icon: typeof FileText; color: string; border: string }> = {
  'Clinical Report': { icon: FileText, color: 'bg-blue-50 text-blue-700', border: 'border-blue-200/70' },
  'Research Paper': { icon: BookOpen, color: 'bg-emerald-50 text-emerald-700', border: 'border-emerald-200/70' },
  Journal: { icon: BookOpen, color: 'bg-purple-50 text-purple-700', border: 'border-purple-200/70' },
  ...
  'Safety Guideline': { icon: ShieldCheck, color: 'bg-red-50 text-red-700', border: 'border-red-200/70' },
};
...
export default function DocumentCard({ title, type, relevance, keyFinding, filePath, pageCount, onPreview }: DocumentCardProps) {
  ...
  // hide the relevance badge + bar for those.
  const hasRelevance = typeof relevance === 'number';
  const relevanceValue = relevance ?? 0;
  ...
  useEffect(() => {
    if (!mounted.current) {
      ...
      setTimeout(() => setBarWidth(relevanceValue), 100);
    }
  }, [relevanceValue]);
  ...
  return (
    ...
    >
      <div className="pointer-events-none absolute inset-x-0 top-0 h-[3px] bg-gradient-to-r from-[#8624FF]/90 to-[#6B1FCC]" />
      ...
      <div className="mb-3 flex items-start justify-between gap-2">
        ...
        <div className="flex items-center gap-2 pr-10">
          ...
          {hasRelevance && (
            <span className={`rounded-full px-2 py-0.5 text-[11px] font-semibold ${getRelevanceBadgeColor(relevanceValue)} ${getRelevanceBadgeBg(relevanceValue)}`}>
              {relevanceValue}%
            </span>
          )}
        </div>
      ...
      {hasRelevance && (
        <div className="relative mb-3 h-[3px] w-full overflow-hidden rounded-full bg-gray-100">
          <div
            className={`absolute inset-y-0 left-0 rounded-full transition-all duration-500 ease-out ${getRelevanceColor(relevanceValue)}`}
